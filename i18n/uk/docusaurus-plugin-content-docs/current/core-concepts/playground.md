...
sidebar_position: 4
title: Playground
description: Playground — це живий, запущений екземпляр Playspec на Marquee.
---

# Playground

**Playground** — це живе, запущене середовище розробки. Це екземпляр [Playspec](/core-concepts/playspec), розгорнутий на [Marquee](/core-concepts/marquee) — ваш код працює на реальній інфраструктурі, доступний за унікальними HTTPS URL-адресами.
...
Наприклад: `pg-42-my-web-app`. Це забезпечує повну ізоляцію контейнерів та мереж між Playground-ами на одному Marquee.
...
- **Гілка за замовчуванням** — Успадковує від Prop
...
Перевизначуйте субдомен за замовчуванням для будь-якого відкритого сервісу. Це критично при запуску кількох Playground з одного Playspec на одному Marquee — кожному потрібні унікальні субдомени.
...
- Дозволяється лише **один Playground** на комбінацію Playspec+Marquee (для запобігання конфліктів томів)
- Томи мають префікс `pv-{playspec_id}-{marquee_id}` для ізоляції
...
Майданчики містять повнофункціональний **веб-термінал SSH** безпосередньо в браузері. Ви можете безпечно підключатися до хостів Marquee та керувати контейнерами без локальних SSH-ключів.
...
| Максимум Playground на Marquee | **100** |
