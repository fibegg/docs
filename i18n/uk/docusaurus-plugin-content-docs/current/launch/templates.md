...
Змінні шаблону — це плейсхолдери у форматі `$$var__NAME`, що замінюються реальними значеннями при запуску з шаблону.

### Типи змінних

| Патерн | Опис |
|--------|------|
| `$$var__NAME` | Значення від користувача — у формі запуску зʼявляється поле вводу, або ж автоматично згенероване значення якщо змінна має `random: true` |
...
| `random` | Ні | Якщо `true`, платформа автоматично генерує значення |
